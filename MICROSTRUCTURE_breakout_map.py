...
Separate historical exploratory research. It does not touch, score, filter or
inspect Exp27 / Decision Calibration fresh-forward ledgers. No candle color,
fitted spread threshold, Last/Volume tape or runtime signal is used.
"""
...
def _valid_ticks(raw: np.ndarray | None) -> np.ndarray:
    ...
    out = raw[good]
    if len(out) > 1 and np.any(np.diff(out["time_msc"].astype(np.int64)) < 0):
        raise RuntimeError("MT5 tick chronology is not non-decreasing")
    return out
...
def _pre_features(ticks: np.ndarray, i: int, direction: int, point: float) -> dict | None:
    ...
    for name, w in (("250ms", 250), ("1s", 1000), ("5s", 5000)):
        ...
        out[f"spread_delta_{name}"] = (
            float(spread[i] - spread_ref) if np.isfinite(spread_ref) else np.nan
        )
...
def _future_labels(
    ...
    rec_cond = (
        (ask[break_i:post_end] < level)
        if direction > 0
        else (bid[break_i:post_end] > level)
    )
    ...
    for target in TARGET_POINTS:
        ...
        if len(target_hits):
            ...
            out[f"time_to_{target}_ms"] = (
                int(ms[ti] - break_ms) if before_rec else np.nan
            )
        else:
...
def _bootstrap_day_diff(df: pd.DataFrame, outcome: str) -> dict:
    ...
    point = float(
        q.loc[q["spread_expanded"], outcome].mean()
        - q.loc[~q["spread_expanded"], outcome].mean()
    )
...
def _two_dim_map(df: pd.DataFrame, outcome: str, stage: str) -> pd.DataFrame:
    ...
    if (
        len(q) < 25
        or q["spread_ratio"].nunique() < 5
        or q["tick_rate_ratio_1s"].nunique() < 5
    ):
        return pd.DataFrame()
...
def _print_rate_table(
    ...
    for expanded in (False, True):
        ...
        vals = " ".join(
            f"{c}={100 * z[c].mean():6.2f}%"
            if len(z) and z[c].notna().any()
            else f"{c}=NA"
            for c in outcomes
        )
...
def main() -> int:
    ...
    try:
        ...
        for day_no, dts in enumerate(days, start=1):
            ...
            q_end = (
                _brt_local(d + timedelta(days=1), time(0, 0, 0))
            ...
            if len(ticks) == 0:
                ...
                if day_no == 1 or day_no % 10 == 0 or day_no == total_days:
                    print(
                        f"SCAN {day_no:>3}/{total_days} {d} ticks=0 "
                        f"events={len(events)} pending={len(pending)-pending_i}"
                    )
                continue
            ...
            for i in range(len(ticks)):
                ...
                # A level only becomes eligible after the existing causal swing
                # confirmation timestamp. If the first observed quote after
                # confirmation is already through the attack side, exclude it
                # conservatively: the level was not demonstrably available
                # before that attack.
                while (
                    pending_i < len(pending)
                    and int(pending[pending_i]["confirm_ms"]) <= tms
                ):
                    ...
                    if int(lv["direction"]) > 0:
                        if ask[i] >= level:
                            status["STALE_AT_CONFIRM"] += 1
                        else:
                            heapq.heappush(res_heap, (level, seq, lv))
                            seq += 1
                    else:
                        if bid[i] <= level:
                            status["STALE_AT_CONFIRM"] += 1
                        else:
                            heapq.heappush(sup_heap, (-level, seq, lv))
                            seq += 1
            ...
            if day_no == 1 or day_no % 10 == 0 or day_no == total_days:
                print(
                    ...
                    f"events={len(events):>5} active={len(res_heap)+len(sup_heap):>5} "
                    f"pending={len(pending)-pending_i:>5}"
                )
        ...
        if not events:
            ...
            print(
                ...
                f"days_no_ticks={days_no_ticks} "
                f"valid_ticks={total_valid_ticks}"
            )
            print("LEVEL STATUS")
        ...
        e = (
            pd.DataFrame(events)
            .sort_values(["attack_time", "tf", "level_price"])
            .reset_index(drop=True)
        )
        ...
        grid = (
            pd.concat(
                [x for x in (grid_a, grid_b) if not x.empty],
                ignore_index=True,
            )
            if (not grid_a.empty or not grid_b.empty)
            else pd.DataFrame()
        )
        ...
        for feat_name in MAP_FEATURES:
            ...
            print(feat_name)
            print(
                m[["bucket", "n", "rate", "feature_mean"]].to_string(
                    index=False,
        ...
        print(
            "NEXT STEP = FREEZE ONE CANDIDATE FINGERPRINT AS MAP02 "
            "ONLY AFTER MAP01 INSPECTION"
        )
