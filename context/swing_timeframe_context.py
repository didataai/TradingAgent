...
def aggregate_market(timeframes: dict[str, dict[str, Any]]) -> dict[str, Any]:
    weights = {"W1": 7, "D1": 6, "H4": 4, "H1": 2, "M15": 1}
    score = 0
    total = 0
    conflicts: list[str] = []
    traces: list[str] = []
    ...
    if "D1" in timeframes and "H4" in timeframes:
        d1 = timeframes["D1"]["state"]["structure"]
        h4 = timeframes["H4"]["state"]["structure"]
        if d1.startswith("BEARISH") and h4.startswith("BULLISH"):
            conflicts.append("H4_CORRECAO_ALTISTA_CONTRA_D1_BAIXISTA")
        if d1.startswith("BULLISH") and h4.startswith("BEARISH"):
            conflicts.append("H4_CORRECAO_BAIXISTA_CONTRA_D1_ALTISTA")

    if "D1" in timeframes and "H1" in timeframes:
        d1 = timeframes["D1"]["state"]["structure"]
        h1 = timeframes["H1"]["state"]["structure"]
        if d1.startswith("BEARISH") and h1.startswith("BULLISH"):
            conflicts.append("H1_REACAO_ALTISTA_CONTRA_D1_BAIXISTA")
        if d1.startswith("BULLISH") and h1.startswith("BEARISH"):
            conflicts.append("H1_REACAO_BAIXISTA_CONTRA_D1_ALTISTA")
    ...
    if "W1" in timeframes:
        h = timeframes["W1"]
        narrative.append(
            f"W1 define o contexto estrutural: {h['state']['structure']}, "
            f"momentum {h['state']['momentum']} e volume {h['state']['volume']}."
        )
    if "D1" in timeframes:
        h = timeframes["D1"]
        narrative.append(
            f"D1 define o viés principal: {h['state']['structure']}, "
            f"estado {h['state']['live_bar']} e volume {h['state']['volume']}."
        )
    if "H4" in timeframes:
        h = timeframes["H4"]
        narrative.append(
            f"H4 mostra a estrutura intermediária: {h['state']['structure']}, "
            f"momentum {h['state']['momentum']} e volume {h['state']['volume']}."
        )
    if "H1" in timeframes:
        h = timeframes["H1"]
        narrative.append(
            f"H1 refina a entrada: {h['state']['structure']}, "
            f"estado {h['state']['live_bar']} e volume {h['state']['volume']}."
        )
    if "M15" in timeframes:
        h = timeframes["M15"]
        narrative.append(
            f"M15 é apoio de timing: {h['state']['structure']}, "
            f"estado {h['state']['live_bar']} e volume {h['state']['volume']}."
        )
    if stale:
        narrative.append(
            "Barras paradas/defasadas em " + ", ".join(stale)
            + "; tratar como provisórias ou fechamento de sessão."
        )
...
def find_default_input(project_root: Path, symbol: str) -> Path:
    ...
    for suffix in (".parquet", ".csv"):
        candidate = base / f"{symbol}_swing{suffix}"
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        f"Não encontrei {symbol}_swing em {base}. Use --input."
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Gera contexto swing compacto por timeframe.")
    parser.add_argument("--input", type=Path, help="Consolidado swing Parquet/CSV")
    ...
    latest_price_ctx = contexts.get("M15") or contexts.get("H1") or contexts.get("H4") or next(iter(contexts.values()))
    payload = {
        "schema_version": "1.3-swing",
    ...
    output_path = args.output or (args.project_root / "data" / "context" / f"{symbol}_swing_context.json")
