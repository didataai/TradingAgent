...
# -*- coding: utf-8 -*-
"""
TradingAgent - Factual Prompt Payload Builder (Intraday)
...
Exemplos:
  ...
  python context/prompt_payload.py --symbol GBPUSD --market-data data/consolidated/GBPUSD_intraday.parquet
"""
...
def find_default_context(project_root: Path, symbol: str) -> Path:
    path = project_root / "data" / "context" / f"{symbol}_intraday_context.json"
    if not path.exists():
...
def find_default_market_data(project_root: Path, symbol: str) -> Path:
    ...
    candidates = (
        base / f"{symbol}_intraday.parquet",
        base / f"{symbol}_intraday.csv",
...
def build_payload(context: dict[str, Any], market_df: pd.DataFrame, market_path: Path) -> dict[str, Any]:
    ...
    return clean({
        ...
        "data_semantics": {
            ...
            "bar_live_rule": "A barra LIVE ainda pode mudar. Barras CLOSED servem como confirmação histórica.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gera payload factual intraday, sem viés decisório.")
    parser.add_argument("--context", type=Path, help="JSON de timeframe_context.py")
    parser.add_argument("--market-data", type=Path, help="Parquet/CSV consolidado intraday")
    ...
    output_path = args.output or (
        args.project_root / "data" / "payload" / f"{symbol}_intraday_payload.json"
    )
