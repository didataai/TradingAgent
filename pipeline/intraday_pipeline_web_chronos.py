...
FINALIDADE
    ...
    e execução do agente intraday ou geração do input para uso via Web.
...
EXEMPLOS
    ...
    python pipeline/intraday_pipeline.py --symbol GOLD --web-agent
...
LIMITAÇÕES
    ...
    - --web-agent gera o arquivo para ChatGPT Web e não chama LLM;
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Gera somente o input completo para uso manual via ChatGPT Web, "
            "sem chamar a LLM local."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Ignora somente o Chronos Runtime e mantém integralmente "
            "o fluxo intraday anterior."
        ),
    )
    parser.add_argument(
        "--no-lock",
...
def main() -> int:
    ...
    if args.skip_agent and args.web_agent:
        print(
            "ERRO: --skip-agent e --web-agent são incompatíveis.",
            file=sys.stderr,
        )
        return 2

    if args.web_agent and args.agent_mode == "ensemble":
        print(
            "ERRO: --web-agent gera um único input e não usa ensemble.",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        logger.write(
            ...
            f"| skip_chronos={args.skip_chronos} "
            ...
            for symbol in symbols:
                ...
                try:
                    if not args.agent_only:
                        ...
                        # Chronos é uma camada opcional. Falhas nunca quebram o
                        # fluxo legado, salvo se required=true for configurado.
                        if not args.skip_chronos:
                            for chronos_step in (
                                "chronos_runtime",
                                "chronos_payload_bridge",
                            ):
                                chronos_cfg = steps_cfg.get(chronos_step)
                                if not chronos_cfg or not chronos_cfg.get("enabled", False):
                                    continue

                                enabled_symbols = [
                                    str(item).upper()
                                    for item in chronos_cfg.get("symbols", [])
                                ]
                                if enabled_symbols and symbol not in enabled_symbols:
                                    logger.write(
                                        "Etapa ignorada | "
                                        f"step={chronos_step} | symbol={symbol} "
                                        "| reason=symbol_not_enabled"
                                    )
                                    continue

                                chronos_result = run_step(
                                    name=chronos_step,
                                    script=resolve_path(chronos_cfg["script"]),
                                    arguments=expand_arguments(
                                        chronos_cfg.get("arguments", []), symbol
                                    ),
                                    timeout_seconds=float(
                                        chronos_cfg.get("timeout_seconds", 120)
                                    ),
                                    logger=logger,
                                    symbol=symbol,
                                )
                                symbol_record["steps"][chronos_step] = chronos_result

                                if not chronos_result["success"]:
                                    required = bool(chronos_cfg.get("required", False))
                                    logger.write(
                                        "Chronos indisponível | "
                                        f"step={chronos_step} | symbol={symbol} "
                                        f"| required={required}"
                                    )
                                    if required:
                                        raise RuntimeError(
                                            f"Etapa {chronos_step} falhou para {symbol}."
                                        )
                                    break
                        else:
                            logger.write(
                                "Etapa ignorada | step=chronos | "
                    ...
                    if args.web_agent:
                        web_cfg = steps_cfg.get(
                            ...
                            {
                                "enabled": True,
                                "script": "agent/web_input_agent.py",
                                "arguments": ["--symbol", "{symbol}"],
                                "timeout_seconds": 60,
                            },
                        )
                        if web_cfg.get("enabled", True):
                            web_arguments = expand_arguments(
                                web_cfg.get("arguments", []),
                                symbol,
                            )
                            if args.analyst:
                                web_arguments.extend(["--analyst", args.analyst])

                            result = run_step(
                                name="web_input_agent",
                                script=resolve_path(web_cfg["script"]),
                                arguments=web_arguments,
                                timeout_seconds=float(
                                    web_cfg.get("timeout_seconds", 60)
                                ),
                                logger=logger,
                                symbol=symbol,
                            )
                            symbol_record["steps"]["web_input_agent"] = result

                            if not result["success"]:
                                raise RuntimeError(
                                    f"Web Input Agent falhou para {symbol}."
                                )
