...
// ----------------------------------------------------------------------------
// PURPOSE
//   Visual microscope for the quote-path quantities studied in Map02/Map03/Map04.
//   This EA NEVER sends orders and NEVER classifies BUY/SELL.
//
// LIVE WINDOW (default 1 second)
//   MID        = (BID + ASK) / 2
//   NET        = abs(MID_now - MID_at_or_before_t_minus_window)
//   TOTAL_PATH = sum(abs(delta MID)) inside the window
//   SUPPORT    = (TOTAL_PATH + NET) / 2
//   OPPOSING   = (TOTAL_PATH - NET) / 2
//   EFFICIENCY = NET / TOTAL_PATH
//
...
input int   InpWindowMs             = 1000;   // Measurement window in milliseconds
input int   InpRefreshMs            = 100;    // Panel refresh cadence
input int   InpMaxBufferedTicks     = 50000;  // In-memory tick capacity
input int   InpSeedHistoryMs        = 6000;   // Initial tick history seed

input group "Panel"
input bool             InpShowPanel        = true;
input ENUM_BASE_CORNER InpPanelCorner      = CORNER_RIGHT_UPPER;
input int              InpPanelX           = 12;
input int              InpPanelY           = 54;
input int              InpPanelWidth       = 310;
...
input group "Current price marker"
input bool  InpDrawCurrentMarker = true;  // One moving marker only; no signal history
input int   InpMarkerBars        = 2;     // Horizontal length in chart bars
...
void EnsureBackground()
{
   ...
   ObjectSetInteger(0, name, OBJPROP_YSIZE, 148);
...
void SetRow(const int row, const string text, const color clr)
{
   ...
   int row_h = InpPanelFontSize + 7;
   int y = InpPanelY + 8 + row * row_h;
...
void RefreshPanel()
{
   ...
   if(!InpShowPanel)
   {
      for(int r = 0; r < 8; ++r) ObjectDelete(0, Obj("ROW_" + IntegerToString(r)));
   ...
   if(!s.ready)
   {
      SetRow(0, "MICRO LIVE  [OBSERVATIONAL]", InpPanelTextColor);
      SetRow(1, "warming tick buffer...", InpFlatColor);
      SetRow(2, "window: " + IntegerToString(InpWindowMs) + " ms", InpPanelTextColor);
      SetRow(3, "NO SIGNAL / NO THRESHOLD", InpFlatColor);
      for(int r = 4; r < 8; ++r) SetRow(r, "", InpPanelTextColor);
      DrawMarker(s);
      ChartRedraw(0);
      return;
   }
   ...
   SetRow(0, "MICRO LIVE  [OBSERVATIONAL]", InpPanelTextColor);
   SetRow(1, "MID " + IntegerToString(InpWindowMs) + "ms: " + Signed1(s.net_signed_points) + " pt  " + DirectionText(s.direction), dclr);
   SetRow(2, "PATH total: " + DoubleToString(s.total_path_points, 1) + " pt   eff: " + Pct1(s.efficiency), InpPanelTextColor);
   SetRow(3, "SUP: " + DoubleToString(s.supporting_path_points, 1) + "   OPP: " + DoubleToString(s.opposing_path_points, 1) + " pt", InpPanelTextColor);
   SetRow(4, "DIR updates: " + Pct1(s.directional_update_fraction) + "   BID+ASK: " + Pct1(s.both_quote_change_fraction), InpPanelTextColor);
   SetRow(5, "Spread: " + DoubleToString(s.spread_points, 1) + " pt   ticks: " + IntegerToString(s.ticks_in_window), InpPanelTextColor);
   SetRow(6, "MID: " + DoubleToString(s.mid, _Digits), dclr);
   SetRow(7, "NO SIGNAL / NO THRESHOLD", InpFlatColor);
...
int OnInit()
{
   ...
   Print("Book5 Microstructure Live View initialized: OBSERVATIONAL ONLY; no orders, no thresholds.");
