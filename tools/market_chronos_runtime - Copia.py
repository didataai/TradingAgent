...
def normalize_event_time(df: pd.DataFrame) -> pd.DataFrame:
    if "event_time" not in df.columns:
        raise ValueError("A base MTF não contém a coluna obrigatória event_time.")
    out = df.copy()
    out["event_time"] = pd.to_datetime(out["event_time"], errors="coerce")
    out = out.dropna(subset=["event_time"]).sort_values("event_time").drop_duplicates("event_time", keep="last")
    if out.empty:
...
def read_runtime_window(path: Path, warmup_bars: int) -> tuple[pd.DataFrame, int]:
    ...
    # Para 100 mil linhas, a leitura integral é barata e permite tail determinístico.
    # O processamento pesado é limitado à janela de aquecimento.
...
def freshness_info(event_time: Any, max_age_minutes: int) -> dict[str, Any]:
    event_ts = parse_timestamp(event_time)
...
def main() -> None:
    ...
    log(f"Processando {len(window)} de {total_rows} linhas")
    ...
    freshness = freshness_info(state.get("event_time"), args.max_age_minutes)
