...
Lê a base consolidada, seleciona exclusivamente candles M1 fechados e
reconstrói M5, M15 e H1 do próprio M1 para evitar mistura e lookahead.
"""
...
def resample_context(m1,tf):
    x=m1.set_index("event_time");o=x.resample(TF_RULES[tf],label="left",closed="left").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna(subset=["open","high","low","close"]).reset_index()
    o["closed_time"]=o.event_time+pd.Timedelta(minutes=TF_MINUTES[tf]);tr=pd.concat([(o.high-o.low),(o.high-o.close.shift()).abs(),(o.low-o.close.shift()).abs()],axis=1).max(axis=1);o["atr"]=tr.rolling(14,min_periods=5).mean();o["volume_ratio"]=o.volume/o.volume.rolling(20,min_periods=5).mean();return anatomy(o)
...
def detect(m1,contexts,horizons,include_double_breaks):
    ...
    for i in range(1,len(m1)-max(horizons)-1):
        ...
        if up and down and not include_double_breaks:continue
        for side in (["UP"] if up and not down else ["DOWN"] if down and not up else ["UP","DOWN"] if up and down else []):
            ...
            e={"event_id":f"M1_{i}_{side}","event_time":t,"side":side,"entry_price":entry,"atr":float(c.atr),"m1_double_break":bool(up and down),"m1_previous_color":p.color,"m1_previous_body_bucket":body_bucket(float(p.body_ratio)),"m1_previous_range_bucket":range_bucket(float(p.range_atr)),"m1_previous_volume_bucket":volume_bucket(float(p.volume_ratio)),"m1_previous_body_ratio":p.body_ratio,"m1_previous_range_atr":p.range_atr,"m1_previous_volume_ratio":p.volume_ratio,"m1_current_color":c.color,"m1_current_body_ratio":c.body_ratio,"m1_current_range_atr":c.range_atr,"m1_current_volume_ratio":c.volume_ratio,"m1_break_distance_atr":abs(float(c.close)-entry)/float(c.atr)}
            aligned=0;known=0
...
def candidates(train,test,keys,horizons,min_train,min_test):
    a=aggregate(train,keys,horizons,min_train);b=aggregate(test,keys,horizons,min_test);k=keys+["horizon_minutes"];m=a.merge(b,on=k,how="outer",suffixes=("_train","_test"));enough=(m.sample_size_train>=min_train)&(m.sample_size_test>=min_test)
    pos=enough&(m.success_rate_train>=.55)&(m.success_rate_test>=.55)&(m.avg_return_atr_train>0)&(m.avg_return_atr_test>0);neg=enough&(m.success_rate_train<.45)&(m.success_rate_test<.45)&(m.avg_return_atr_train<0)&(m.avg_return_atr_test<0)
...
def parse_args():
    p=argparse.ArgumentParser();p.add_argument("--symbol",default="GOLD");p.add_argument("--source",default=DEFAULT_SOURCE);p.add_argument("--output",default=DEFAULT_OUTPUT);p.add_argument("--horizons-minutes",nargs="+",type=int,default=[5,15,30,60]);p.add_argument("--train-ratio",type=float,default=.7);p.add_argument("--min-sample",type=int,default=200);p.add_argument("--min-train",type=int,default=200);p.add_argument("--min-test",type=int,default=80);p.add_argument("--include-double-breaks",action="store_true");return p.parse_args()
def main():
    ...
    log(f"Lendo base consolidada: {source}");m1=load_m1(source,symbol);contexts={tf:resample_context(m1,tf) for tf in TF_RULES};log(f"Candles M1: {len(m1)} | M5={len(contexts['M5'])} | M15={len(contexts['M15'])} | H1={len(contexts['H1'])}")
    hs=sorted(set(a.horizons_minutes));ev=split(detect(m1,contexts,hs,a.include_double_breaks),a.train_ratio);train=ev[ev.temporal_split=="TRAIN"];test=ev[ev.temporal_split=="TEST"]
    ...
    meta={"script":"market_volatility_dna.py","version":"1.2-consolidated-research-base","generated_at_utc":datetime.now(timezone.utc).isoformat(),"symbol":symbol,"source":str(source),"m1_start":m1.event_time.min(),"m1_end":m1.event_time.max(),"m1_candles":len(m1),"context_candles":{tf:len(f) for tf,f in contexts.items()},"context_color_distribution":{tf:contexts[tf].color.value_counts().to_dict() for tf in contexts},"events":len(ev),"double_breaks_included":bool(a.include_double_breaks),"train_events":len(train),"test_events":len(test),"horizons_minutes":hs,"min_train":a.min_train,"min_test":a.min_test,"stable_positive_candidates":int((cand.candidate_status=="STABLE_POSITIVE").sum()),"stable_negative_candidates":int((cand.candidate_status=="STABLE_NEGATIVE").sum()),"output":str(out)};save_json(out/"metadata.json",meta);log("OK");print(json.dumps(clean(meta),ensure_ascii=False,indent=2))
