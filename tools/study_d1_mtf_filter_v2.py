...
def strategy_masks(df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    masks.update({
        ...
        "D1_EXTREME_HIGH_BUY_CHASE": baseline & df["zone"].eq("EXTREME_HIGH") & df["side"].eq("BUY"),
        "D1_EXTREME_LOW_SELL_CHASE": baseline & df["zone"].eq("EXTREME_LOW") & df["side"].eq("SELL"),
    })
    return masks
...
def assess_candidate(report: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    if not days_ok:
        reasons.append("INSUFFICIENT_INDEPENDENT_TEST_DAYS_LT_10")
...
def build_anchor(input_dir: Path, symbol: str, rules: dict[str, Any]) -> tuple[pd.DataFrame, pd.Timestamp, pd.Timestamp, dict[str, int]]:
    m5 = v1.add_future_outcomes(v1.add_d1_point_in_time(v1.prepare(input_dir / f"{symbol}_M5.parquet", "M5", rules)))
    m15 = v1.prepare(input_dir / f"{symbol}_M15.parquet", "M15", rules)
    h1 = v1.prepare(input_dir / f"{symbol}_H1.parquet", "H1", rules)
    ...
    zones = [
        ...
        for p, d in zip(anchor["d1_position"], anchor["daily_direction"])
    ]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="D1 broker-day + H1/M15/M5 study v2 with side split and day-cluster validation.")
    ...
    anchor, split_a, split_b, coverage = build_anchor(args.input_dir, symbol, rules)
    ...
    report: dict[str, Any] = {
        ...
        "methodology": {
            ...
            "promotion_guard": "hard candidate requires >=50 test events and >=10 independent broker days",
        },
        "coverage": {
            **coverage,
    ...
    print(json.dumps({
        ...
        "baseline_broker_days": report["coverage"]["baseline_broker_days"],
